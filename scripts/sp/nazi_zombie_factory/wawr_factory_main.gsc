...
init()
{
	set_zombie_var( "tesla_upgraded_max_arcs",			12 );
	set_zombie_var( "tesla_upgraded_max_enemies_killed", 24 );
	set_zombie_var( "tesla_upgraded_radius_start",		800 );
	set_zombie_var( "tesla_upgraded_radius_decay",		10 );
}
...
tesla_arc_damage_override( source_enemy, player, arc_num )
{
	...
	maps\_zombiemode_tesla::tesla_flag_hit( self, true );
	wait_network_frame();
	self thread maps\_zombiemode_tesla::tesla_do_damage( source_enemy, arc_num, player );

	radius_decay = level.zombie_vars["tesla_radius_decay"] * arc_num;
	enemies = maps\_zombiemode_tesla::tesla_get_enemies_in_area( self GetTagOrigin( "j_head" ), level.zombie_vars["tesla_radius_start"] - radius_decay, player );
	maps\_zombiemode_tesla::tesla_flag_hit( enemies, true );
	...
	for( i = 0; i < enemies.size; i++ )
	{
		...
		if ( maps\_zombiemode_tesla::tesla_end_arc_damage( arc_num + 1, player.tesla_enemies_hit, is_upgraded ) )
		{			
			maps\_zombiemode_tesla::tesla_flag_hit( enemies[i], false );
			continue;
		}

		player.tesla_enemies_hit++;
		enemies[i] maps\_zombiemode_tesla::tesla_arc_damage( self, player, arc_num + 1 );
	}
}
...
tesla_end_arc_damage_override( arc_num, enemies_hit_num, is_upgraded )
{
	if ( is_upgraded )
	{
		if ( arc_num >= level.zombie_vars["tesla_upgraded_max_arcs"] )
		{
			return true;
		}
		if ( enemies_hit_num >= level.zombie_vars["tesla_upgraded_max_enemies_killed" ] )
		{	
			return true;
		}
		radius_decay = level.zombie_vars["tesla_upgraded_radius_decay"] * arc_num;
		if ( level.zombie_vars["tesla_upgraded_radius_start"] - radius_decay <= 0 )
		{
			return true;
		}
	}
